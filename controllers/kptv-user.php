    ...
    class KPTV_User extends \KPT\Database
    {
        ...
        public function validate_user(): void
        {
            ...
            try {
                ...
                $success = $this->query('UPDATE kptv_users SET u_active = 1, u_hash = "", u_updated = CURRENT_TIMESTAMP WHERE id = ?')
                    ->bind([$user->id])
                    ->execute();
        ...
        private function authenticateUser(string $username, string $password): void
        {
            ...
            $this->query('UPDATE kptv_users SET login_attempts = 0, locked_until = NULL, last_login = CURRENT_TIMESTAMP WHERE id = ?')
                ->bind([$user->id])
                ->execute();
        ...
        {
            $lockUntil = gmdate('Y-m-d H:i:s', time() + self::LOCKOUT_TIME);
            ...
            $this->query(
                'UPDATE kptv_users SET 
                    ...
                    END
                WHERE id = ?'
            )->bind([self::MAX_LOGIN_ATTEMPTS, $lockUntil, $userId])
                ->execute();
        }
        ...
        private function updatePassword(int $userId, string $newPassword): void
        {
            ...
            $success = $this->query('UPDATE kptv_users SET u_pass = ?, u_updated = CURRENT_TIMESTAMP WHERE id = ?')
        ...
        private function rehash_password(int $userId, string $password): void
        {
            ...
            $this->query('UPDATE kptv_users SET u_pass = ?, u_updated = CURRENT_TIMESTAMP WHERE id = ?')
                ->bind([$passwordHash, $userId])
                ->execute();
        }
        ...
        private function update_user(array $data, int $currentUserId): void
        {
            ...
            // execute the update
            $this->query('UPDATE kptv_users SET u_fname = ?, u_lname = ?, u_email = ?, u_role = ?, u_updated = CURRENT_TIMESTAMP WHERE id = ?')
                ->bind([
                    $data['u_fname'],
                    $data['u_lname'],
                    $data['u_email'],
