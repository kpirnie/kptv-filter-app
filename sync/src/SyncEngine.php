        ...
        if (!empty($rawStreams)) {
            ...
            foreach ($rawStreams as $stream) {
                ...
                if (count($tempRecords) >= 1000) {
                    ...
                    $insertedCount += count($tempRecords);
                    $tempRecords = [];
                }
            }
    ...
    private function syncTempToStreams(int $userId, int $providerId): int
    {
        ...
        if (!empty($uriUpdates)) {
            ...
            foreach ($uriUpdates as [$streamId, $uri]) {
                $this->db->execute_raw(
                    "UPDATE kptv_streams SET s_stream_uri = ?, s_updated = s_updated WHERE id = ?",
                    [$uri, $streamId]
                );
                $updateCount++;
        ...
        if (!empty($nameUpdates)) {
            ...
            foreach ($nameUpdates as [$streamId, $name]) {
                $this->db->execute_raw(
                    "UPDATE kptv_streams SET s_orig_name = ?, s_updated = s_updated WHERE id = ?",
                    [$name, $streamId]
                );
                $updateCount++;
                if ($updateCount % 500 === 0) {
