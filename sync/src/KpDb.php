...
class KpDb
{
    ...
    public function __construct(
        ...
        if ($normalizedDriver === 'sqlite') {
            ...
            unset($settings->server, $settings->port, $settings->schema, $settings->username, $settings->password, $settings->charset, $settings->collation);
        }
    ...
    public function insert_many(
        ...
        foreach ($batches as $batch) {
            ...
            try {
                $valuesPlaceholders = implode(', ', array_fill(0, count($batch), $rowPlaceholder));
                $query = "INSERT{$ignoreKeyword} INTO {$fullTable} ({$columnList}) VALUES {$valuesPlaceholders}";
